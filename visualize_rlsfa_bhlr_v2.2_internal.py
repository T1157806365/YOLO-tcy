...
"""
RLSFA + BHLR-v2 internal visualization
=======================================

Designed for the current BHLR-v2 + SENet selector:
    models/modules/bhlr_v2.py
    models/rgbt_rlsfa_bhlr_model_v2.py
    val_rlsfa_bhlr_v2.py

Visualization policy
--------------------
1. Every result is saved as a standalone JPG; no grouped panels / overview.
2. No title strip is drawn over the image.
3. Network feature heatmaps cover the complete LetterBox network canvas,
   including padding regions, because those regions also have CNN features.
4. Targetness / reliability use fixed [0,1].
5. Selector multiplier uses fixed [0,2].
6. Images 41/42 share exactly the same color range.
7. Image 44 is the true signed response change:
       mean_c(|F_after|) - mean_c(|F_before|).
8. New SENet diagnostics are appended as 46-49 so the existing 00-45
   numbering remains unchanged:
       46 selector feature response BEFORE SENet
       47 selector feature response AFTER SENet
       48 SENet six-group mean channel weights
       49 SENet all-channel weights
...
def shared_robust_range(
    *arrays: np.ndarray,
    ...
    """One color range shared by paired before/after maps."""
    vals = []
    for a in arrays:
...
def signed_zero_centered_overlay(
    base_lb: np.ndarray,
    signed_map: np.ndarray,
    orig_hw: Tuple[int, int],
    ratio_pad=None,
    alpha_max: float = 0.78,
    percentile: float = 99.0,
) -> Tuple[np.ndarray, float]:
    """
    Zero-centered signed overlay for quantities such as selector logit Z3.

    signed_map > 0  -> red
    signed_map < 0  -> blue
    signed_map ~= 0 -> keep original image nearly unchanged

    A symmetric robust limit is estimated from percentile(|signed_map|).
    The map is rendered over the FULL LetterBox canvas.
    """
    base = black_letterbox_padding(
        base_lb,
        orig_hw=orig_hw,
    ...
    h, w = base.shape[:2]
    x = cv2.resize(
        np.asarray(signed_map, dtype=np.float32),
        (w, h),
        interpolation=cv2.INTER_LINEAR,
    )
    x = np.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    if finite.size:
        limit = float(np.percentile(np.abs(finite), float(percentile)))
        if limit <= 1e-12:
    ...
    s = np.clip(x / limit, -1.0, 1.0)

    color = np.zeros_like(base, dtype=np.float32)
    color[..., 2] = 255.0 * np.clip(s, 0.0, 1.0)   # positive -> red
    color[..., 0] = 255.0 * np.clip(-s, 0.0, 1.0)  # negative -> blue

    magnitude = np.abs(s)
    alpha = (magnitude ** 0.85)[..., None] * float(alpha_max)

    out = (
        base.astype(np.float32) * (1.0 - alpha)
        + color * alpha
    )
    return np.clip(out, 0, 255).astype(np.uint8), limit
...
def render_senet_group_weights(
    weights: np.ndarray,
    canvas_hw: Tuple[int, int],
    group_names: Sequence[str] = ("r", "t", "|r-t|", "d", "sr", "st"),
) -> np.ndarray:
    """
    Render six mean SE channel weights as a standalone diagnostic image.

    Standard SENet uses sigmoid weights in [0,1]. Each of the six selector
    groups contains the same number of channels (normally 24).
    """
    ...
    vals = np.asarray(weights, dtype=np.float32).reshape(-1)
    n = len(group_names)
    if vals.size != n:
        raise ValueError(f"Expected {n} group weights, got {vals.size}")

    left = max(70, int(round(w * 0.18)))
    right = max(30, int(round(w * 0.06)))
    top = max(45, int(round(h * 0.10)))
    bottom = max(45, int(round(h * 0.10)))
    usable_w = max(1, w - left - right)
    usable_h = max(1, h - top - bottom)
    row_h = usable_h / float(n)

    # Axis 0..1 because standard SE weights are sigmoid outputs.
    cv2.line(canvas, (left, top), (left, h-bottom), (180,180,180), 1, cv2.LINE_AA)
    cv2.line(canvas, (left, h-bottom), (w-right, h-bottom), (180,180,180), 1, cv2.LINE_AA)

    for tick in (0.0, 0.25, 0.5, 0.75, 1.0):
        x = int(round(left + tick * usable_w))
        cv2.line(canvas, (x, h-bottom), (x, h-bottom+6), (180,180,180), 1, cv2.LINE_AA)
        cv2.putText(
            canvas, f"{tick:.2f}", (max(0, x-16), h-bottom+24),
            cv2.FONT_HERSHEY_SIMPLEX, 0.42, (220,220,220), 1, cv2.LINE_AA,
        )

    for i, (name, val) in enumerate(zip(group_names, vals)):
        cy = int(round(top + (i + 0.5) * row_h))
        bar_h = max(8, int(round(row_h * 0.42)))
        x2 = int(round(left + float(np.clip(val, 0.0, 1.0)) * usable_w))
        cv2.rectangle(canvas, (left, cy-bar_h//2), (x2, cy+bar_h//2), (210,210,210), -1)
        cv2.putText(
            canvas, name, (12, cy+5),
            cv2.FONT_HERSHEY_SIMPLEX, 0.48, (240,240,240), 1, cv2.LINE_AA,
        )
        cv2.putText(
            canvas, f"{float(val):.4f}", (min(w-90, x2+8), cy+5),
...
def render_senet_channel_weights(
    weights: np.ndarray,
    canvas_hw: Tuple[int, int],
) -> np.ndarray:
    """Render all SE channel weights as a 2D strip image in fixed [0,1]."""
    h, w = int(canvas_hw[0]), int(canvas_hw[1])
    vals = np.asarray(weights, dtype=np.float32).reshape(1, -1)
    vals = np.clip(vals, 0.0, 1.0)
    strip = cv2.resize(vals, (w, h), interpolation=cv2.INTER_NEAREST)
    return colormap_from_norm(strip)
...
def visualize_one(
    ...
    # ---------------------------------------------------------------------
    # Capture the NEW BHLR-v2 SENet internals without changing model debug API.
    # Existing 00-45 outputs still use the normal model debug dictionary.
    # ---------------------------------------------------------------------
    bhlr_module = getattr(model, "bhlr", None)
    if bhlr_module is None:
        bhlr_module = getattr(model, "rsdt", None)
    if bhlr_module is None or not hasattr(bhlr_module, "adapters"):
        raise AttributeError("Cannot locate BHLR-v2 adapters on model.bhlr/model.rsdt")

    adapter_key = str(int(bhlr_scale))
    if adapter_key not in bhlr_module.adapters:
        raise KeyError(
            f"BHLR scale P{bhlr_scale} not found. Available={list(bhlr_module.adapters.keys())}"
        )

    selector = bhlr_module.adapters[adapter_key].selector
    if not hasattr(selector, "channel_attention"):
        raise AttributeError(
            "Current BHLR-v2 selector has no channel_attention. "
            "Use the SENet-enabled bhlr_v2.py checkpoint/model."
        )

    se_capture = {}

    def _capture_se_module(module, inputs, output):
        se_capture["pre"] = inputs[0].detach()
        se_capture["post"] = output.detach()

    def _capture_se_weights(module, inputs, output):
        se_capture["weights"] = output.detach()

    h_se = selector.channel_attention.register_forward_hook(_capture_se_module)
    h_w = selector.channel_attention.fc.register_forward_hook(_capture_se_weights)
    try:
        outputs = model(
            batch["rgb_img"],
            batch["tir_img"],
    ...
    if not all(k in se_capture for k in ("pre", "post", "weights")):
        raise RuntimeError(
            f"Failed to capture SENet internals. Captured={list(se_capture.keys())}"
        )
    ...
    # -------------------------------------------------------------------------
    # BHLR-v2 selector / SENet 36-44
    #
    # Logical order:
    #   selector inputs -> SENet weights -> post-SE features -> Z3 -> M3 -> D_use
    # -------------------------------------------------------------------------
    se_pre = se_capture["pre"]
    se_post = se_capture["post"]
    se_weights_t = se_capture["weights"]

    se_pre_map = tensor_feature_map(se_pre)
    se_post_map = tensor_feature_map(se_post)
    se_shared_vmin, se_shared_vmax = shared_robust_range(
        se_pre_map,
        se_post_map,
    )

    save_native(
        out_dir, 36, "selector_pre_senet",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                se_pre_map,
        ...
        "Selector features before SENet",
        f"shared range with 39=[{se_shared_vmin:.6f}, {se_shared_vmax:.6f}]",
    )
    ...
    if se_weights.size % 6 != 0:
        raise ValueError(
            f"SENet channels must be divisible by 6, got {se_weights.size}"
        )

    se_group_channels = se_weights.size // 6
    se_group_weights = (
        se_weights.reshape(6, se_group_channels).mean(axis=1)
    )

    save_native(
        out_dir, 37, "senet_group_weights",
        render_senet_group_weights(
            se_group_weights,
            canvas_hw=sem_net_hw,
        ),
        "SENet mean weights for six selector groups",
        "r / t / |r-t| / d / sr / st; sigmoid weights [0,1]",
    )

    save_native(
        out_dir, 38, "senet_channel_weights",
        render_senet_channel_weights(
            se_weights,
            canvas_hw=sem_net_hw,
        ),
        "All SENet selector channel weights",
        f"channels={se_weights.size}; fixed [0,1]",
    )

    save_native(
        out_dir, 39, "selector_post_senet",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                se_post_map,
        ...
        "Selector features after SENet",
        f"shared range with 36=[{se_shared_vmin:.6f}, {se_shared_vmax:.6f}]",
    )

    # Z3 uses a true zero-centered diverging overlay.
    selector_logit_t = bhlr["selector_logit"]
    selector_logit = tensor_single_map(selector_logit_t)

    selector_logit_img, selector_logit_limit = signed_zero_centered_overlay(
        rgb_sem_canvas,
        selector_logit,
        orig_hw=rgb_orig_hw,
        ratio_pad=rgb_sem_ratio_pad,
    ...
    selector_logit_img = draw_boxes(
        selector_logit_img,
        rgb_sem_boxes,
    )

    save_native(
        out_dir, 40, "selector_logit",
        selector_logit_img,
        "BHLR-v2 detail-selector logit Z3",
        (
            "red=Z3>0 enhance; blue=Z3<0 suppress; near-zero keeps RGB; "
            f"symmetric |Z3| 99% limit={selector_logit_limit:.6f}; "
            f"min={selector_logit.min():+.4f}, "
            f"mean={selector_logit.mean():+.4f}, "
            f"max={selector_logit.max():+.4f}"
        ),
    )
    ...
    save_native(
        out_dir, 41, "selector_multiplier",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                selector_multiplier,
        ...
        "BHLR-v2 centered detail multiplier M3",
        (
            "M3=1+tanh(Z3); fixed [0,2]; "
            f"min={selector_multiplier.min():.4f}, "
            f"mean={selector_multiplier.mean():.4f}, "
            f"max={selector_multiplier.max():.4f}"
        ),
    )

    save_native(
        out_dir, 42, "selector_multiplier_overlay",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                selector_multiplier,
    ...
    save_native(
        ...
        "BHLR-v2 usable high-resolution detail",
        "D_use = M3 * D3",
    )

    save_native(
        out_dir, 44, "usable_detail_overlay",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                tensor_feature_map(usable_detail),
    ...
    # -------------------------------------------------------------------------
    # BHLR-v2 result 45-48
    # -------------------------------------------------------------------------
    before_map = tensor_feature_map(raw_rgb_p3)
    after_map = tensor_feature_map(enhanced_rgb_p3)
    shared_vmin, shared_vmax = shared_robust_range(
        before_map,
        after_map,
    )

    save_native(
        out_dir, 45, "rgb_p3_before_bhlr",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                before_map,
    ...
    delta_img = draw_boxes(
        delta_img,
        rgb_sem_boxes,
    )
    save_native(
        out_dir, 48, "bhlr_response_increase_decrease",
        delta_img,
        "BHLR-v2 true response increase/decrease",
        (
            "red=increase; blue=decrease; "
            f"symmetric 99% limit={response_delta_limit:.6f}"
        ),
    )
    ...
    metadata = {
        ...
        "export_policy": {
            ...
            "bhlr_selector_order": [
                "33_bhlr_guidance_targetness",
                "34_bhlr_rgb_structure",
                "35_bhlr_tir_structure",
                "36_selector_pre_senet",
                "37_senet_group_weights",
                "38_senet_channel_weights",
                "39_selector_post_senet",
                "40_selector_logit",
                "41_selector_multiplier",
                "42_selector_multiplier_overlay",
                "43_usable_detail",
                "44_usable_detail_overlay",
                "45_rgb_p3_before_bhlr",
                "46_rgb_p3_after_bhlr",
                "47_bhlr_enhancement",
                "48_bhlr_response_increase_decrease",
                "49_prediction_vs_gt"
            ],
        },
        "rlsfa": {
        ...
        "bhlr_v2": {
            ...
            "selector_logit": {
                ...
                "signed_display_limit_99pct": float(selector_logit_limit),
                "display_semantics": "red=positive, blue=negative, unchanged RGB=near zero",
            },
            "selector_multiplier": {
            ...
            "senet": {
                "num_channels": int(se_weights.size),
                "group_channels": int(se_group_channels),
                "group_names": ["r", "t", "abs_r_minus_t", "d", "sr", "st"],
                "group_mean_weights": [float(v) for v in se_group_weights],
                "weight_min": float(se_weights.min()),
                "weight_mean": float(se_weights.mean()),
                "weight_max": float(se_weights.max()),
                "pre_post_shared_color_range": {
                    "vmin": float(se_shared_vmin),
                    "vmax": float(se_shared_vmax),
                },
                "z3_output_bias": False,
            },

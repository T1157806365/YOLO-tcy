"""Full-feature validator for RLSFA + BHLR-v2, reusing val_rsdt.py."""
...
def load_rlsfa_bhlr_v2_checkpoint(
    checkpoint_path: str,
    device: torch.device,
):
    ...
    print("\nLoading RLSFA + BHLR-v2 checkpoint:")
    print(path)
    ...
    ratio = int(
        model_cfg.get(
            ...
            max(1, high_imgsz // semantic_imgsz),
        )
    )
    ...
    model.load_state_dict(
        _strip_module_prefix(ckpt["model_state_dict"]),
        strict=True,
    )
    ...
    print(f"BHLR-v2 scales: {list(model.bhlr_scales)}")
    print(f"BHLR guidance: {getattr(model, 'bhlr_guidance_mode', 'auto')}")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Full validation for RGB-T + RLSFA + BHLR-v2"
    )

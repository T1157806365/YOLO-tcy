...
class CrossModalDetailSelector(nn.Module):
    ...
    Z3/M3 decision inputs are projected to a small channel width first:
        RGB semantic
        TIR semantic
        |RGB-TIR|
        RGB structure
    ...
    IMPORTANT:
        High-resolution detail D_s / D3 is NOT used to generate Z3/M3.
        D3 is kept only as the detail CONTENT that is finally modulated by M3:
            usable_detail = M3 * D3

    The five projected decision groups and targetness are first fused by the
    existing 1x1 Conv-BN-SiLU block. SENet is then applied to the fused guide
    feature before the existing depthwise spatial refinement.
    ...
    def __init__(
        ...
        # [r,t,|r-t|,sr,st,targetness] -> 1x1 Conv-BN-SiLU -> g channels
        # -> SENet -> DWConv -> Z3.
        #
        # D3 is deliberately excluded from this decision path.
        self.channel_attention = SENetChannelAttention(
            channels=g,
            reduction=se_reduction,
        )

        # [r,t,|r-t|,sr,st] -> 5*g; plus targetness -> 1.
    ...
    def forward(
        ...
        # Z3 decision input intentionally excludes D3/detail_scale.
        ...
        # Repositioned SENet:
        # 121ch -> existing 1x1 Conv-BN-SiLU -> 24ch -> SENet

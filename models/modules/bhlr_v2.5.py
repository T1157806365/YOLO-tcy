...
class CrossModalDetailSelector(nn.Module):
    ...
    Z3/M3 decision uses ONLY:
        r : projected RGB P3 semantic feature
        t : projected aligned-TIR P3 semantic feature
        A : targetness

    IMPORTANT:
        |r-t| is NOT used in Z3/M3 generation.
        RLSFA structures BR/BT (sr/st) are NOT used in Z3/M3 generation.
        High-resolution detail D_s / D3 is NOT used in Z3/M3 generation.

        D3 is kept only as the detail CONTENT that is finally modulated by M3:
            usable_detail = M3 * D3

    The two projected semantic groups and targetness are first fused by the
    existing 1x1 Conv-BN-SiLU block. SENet is then applied to the fused
    24-channel feature before the existing depthwise spatial refinement.
    ...
    def __init__(
        ...
        # [r,t,targetness] -> 1x1 Conv-BN-SiLU -> g channels
        # -> SENet -> DWConv -> Z3.
        #
        # |r-t|, BR/BT (sr/st), and D3 are deliberately excluded from
        # this Z3/M3 decision path.
        self.channel_attention = SENetChannelAttention(
            channels=g,
            reduction=se_reduction,
        )

        # [r,t] -> 2*g; plus targetness -> 1.
        # With guide_channels=24: 24 + 24 + 1 = 49 channels.
    ...
    def forward(
        ...
        # Z3/M3 decision uses ONLY r, t and targetness A.
        ...
        # Repositioned SENet:
        # 49ch -> existing 1x1 Conv-BN-SiLU -> 24ch -> SENet
...
class BHLRScaleAdapter(nn.Module):
    ...
    def forward(
        ...
        # BR/BT may still be prepared for RLSFA/BHLR debug compatibility,
        # but CrossModalDetailSelector intentionally consumes only:
        #   RGB P3, aligned TIR P3, and targetness A.
        (
            guidance_source,
            guidance,
        ...
        # D3/detail_scale supplies detail CONTENT only.
        # It no longer participates in Z3/M3 decision generation.
        usable_detail = (
            selector_multiplier
            * detail_scale
        )

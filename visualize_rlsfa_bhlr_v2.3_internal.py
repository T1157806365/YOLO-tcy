...
6. Images 44/45 share exactly the same color range.
7. Image 47 is the true signed response change:
       mean_c(|F_after|) - mean_c(|F_before|).
8. SENet is now located AFTER the selector's existing 1x1 Conv-BN-SiLU
   fusion and BEFORE the depthwise 3x3 refinement:
       145ch selector input
           -> 1x1 Conv-BN-SiLU
           -> 24ch fused feature
           -> SENet
           -> 24ch reweighted feature
           -> DWConv 3x3
           -> 1x1 Conv
           -> Z3
...
python visualize_rlsfa_bhlr_v2_internal.py \
  --weights runs/rlsfa_bhlr_v2/yolo26n_UAVCB_RLSFA-P3_BHLRv2-P3_v2_rgb1280_sem640_tir640_seed0_v3/weights/best.pt \
  --split test \
  --device 5 \
...
def render_senet_channel_weights(
    ...
    """Render the 24 fused-feature SE channel weights as a 2D strip in fixed [0,1]."""
...
def visualize_one(
    ...
    # ---------------------------------------------------------------------
    # Capture BHLR-v2 SENet internals without changing model debug API.
    # With the repositioned selector, channel_attention receives the 24ch
    # fused feature AFTER head[0] (1x1 Conv-BN-SiLU) and BEFORE head[1]
    # (DWConv 3x3).
    # ---------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # BHLR-v2 selector / SENet 36-43
    #
    # Actual forward order after SENet reposition:
    #   [r,t,|r-t|,d,sr,st,A]
    #       -> head[0]: 1x1 Conv-BN-SiLU -> fused 24ch
    #       -> SENet(24ch)
    #       -> head[1]: DWConv 3x3
    #       -> head[2]: 1x1 Conv
    #       -> Z3 -> M3 -> D_use
    # -------------------------------------------------------------------------
    se_pre = se_capture["pre"]     # 24ch fused feature before SENet
    se_post = se_capture["post"]   # 24ch fused feature after SENet
    ...
    # 36: fused selector feature immediately BEFORE SENet.
    save_native(
        out_dir, 36, "selector_fused_before_senet",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                se_pre_map,
        ...
        "Selector fused feature before SENet",
        (
            "head[0] output: 1x1 Conv-BN-SiLU; "
            f"shape={tuple(se_pre.shape)}; "
            f"shared range with 38=[{se_shared_vmin:.6f}, {se_shared_vmax:.6f}]"
        ),
    )

    # 37: all 24 SENet weights. There is NO six-group interpretation anymore.
    se_weights = (
        se_weights_t[0, :, 0, 0]
        .detach()
        .float()
    ...
    save_native(
        out_dir, 37, "senet_channel_weights",
        render_senet_channel_weights(
            se_weights,
            canvas_hw=sem_net_hw,
        ),
        "SENet weights on fused selector channels",
        (
            f"channels={se_weights.size}; sigmoid weights [0,1]; "
            "these channels are already cross-source fused and cannot be "
            "assigned separately to r/t/|r-t|/d/sr/st"
        ),
    )

    # 38: the same fused feature AFTER SENet.
    # 36/38 use exactly the same display range.
    save_native(
        out_dir, 38, "selector_fused_after_senet",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                se_post_map,
        ...
        "Selector fused feature after SENet",
        (
            f"shape={tuple(se_post.shape)}; "
            f"shared range with 36=[{se_shared_vmin:.6f}, {se_shared_vmax:.6f}]"
        ),
    )

    # 39: Z3 with a true zero-centered signed overlay.
    ...
    # 41: keep a dedicated RGB-background multiplier overlay.
    save_native(
        out_dir, 41, "selector_multiplier_overlay",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                selector_multiplier,
    ...
    # 42/43: selected high-resolution detail.
    save_native(
        out_dir, 42, "usable_detail",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
    ...
    save_native(
        out_dir, 43, "usable_detail_overlay",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                tensor_feature_map(usable_detail),
    ...
    # 44/45 MUST share one range.
    save_native(
        out_dir, 44, "rgb_p3_before_bhlr",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
                before_map,
    ...
    # 46: actual injected feature tensor.
    enhancement = enhanced_rgb_p3 - raw_rgb_p3
    save_native(
        out_dir, 46, "bhlr_enhancement",
        draw_boxes(
            network_feature_overlay(
                rgb_sem_canvas,
    ...
    # 47: TRUE response increase/decrease.
    ...
    save_native(
        out_dir, 47, "bhlr_response_increase_decrease",
        delta_img,
    ...
    metadata = {
        ...
        "export_policy": {
            ...
            "bhlr_selector_order": [
                ...
                "36_selector_fused_before_senet",
                "37_senet_channel_weights",
                "38_selector_fused_after_senet",
                "39_selector_logit",
                "40_selector_multiplier",
                "41_selector_multiplier_overlay",
                "42_usable_detail",
                "43_usable_detail_overlay",
                "44_rgb_p3_before_bhlr",
                "45_rgb_p3_after_bhlr",
                "46_bhlr_enhancement",
                "47_bhlr_response_increase_decrease",
                "48_prediction_vs_gt"
            ],
        },
        "rlsfa": {
        ...
        "bhlr_v2": {
            ...
            "senet": {
                "position": "after selector head[0] 1x1 Conv-BN-SiLU, before head[1] DWConv3x3",
                "input_feature": "24-channel fused selector feature",
                "num_channels": int(se_weights.size),
                "channel_weights": [float(v) for v in se_weights.tolist()],
